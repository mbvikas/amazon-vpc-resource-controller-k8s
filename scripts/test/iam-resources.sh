...
# Cretes the necessary Roles and Policies required
# to test the VPC Resource Controller on DataPlane

set -eo pipefail

USAGE=$(cat << 'EOM'
Usage: create-iam-resources.sh -s [resource-suffix]

Creates the IAM Policies required by the VPC Resource
Controller to Run.

 Required: [Only one of the flag must be set]
 -n   name of cluster for creating IAM Role for Service Account
 -o   create: To create the IAM Roles required by vpc-resource-controller
