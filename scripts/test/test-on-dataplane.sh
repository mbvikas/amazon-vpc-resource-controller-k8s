...
# The Scripts creates an eksctl cluster and run integration test
# for Windows and Security Group for Pods. The script will only
# run successfully if the account on which it runs is allowlisted
# for ENI Trunking feature. See README for more details.

set -eo pipefail

USAGE=$(cat << 'EOM'
Usage: test-on-dataplane.sh -n [cluster-name] -r [user-role-arn]
...
while getopts "n:i:r:s:" o; do
  case "${o}" in
    ...
    r) # Region where ECR image will be hosted
      AWS_REGION=${OPTARG}
      ;;
    s) # Resource Suffic attached to AWS Resource Names
...
# Only accounts allowlisted for ENI Trunking will be able to run this successfully
